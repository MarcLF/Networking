...
TEST(BoostAsioAsyncTests, AsyncEchoServerHandlesData)
{
	// Port 0 dynamically assigns a port
	TcpAsyncServer server(0);

	// 1. Run the Asio event loop on a background thread
	...
	// 3. Send a message
	std::string test_message = "Hello Neptune, Async Edition!";
	...
	// 5. Cleanup
	client_socket.close();
	server.Stop(); // Tells io_context.run() to return
